...
const projectTooltip = "View on GitHub";

export default function SubsiteProjects() {
	return (
		<React.Fragment>
			<CodeRounded /> <Typography display="inline" variant="h4">Projects</Typography>
			...
			<Box sx={{ display: 'flex', flexDirection: 'row', flexWrap: 'wrap', alignContent: 'space-evenly', gap: 1}}>
				{
					projectTitles.map((_text, index) => (
						<ProjectCard key={projectTitles[index]} img={projectImages[index]} title={projectTitles[index]} description={projectDescriptions[index]} link={projectGithubLinks[index]} tooltip={projectTooltip} />
					))
				}
			</Box>
		</React.Fragment>
